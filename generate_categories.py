...
def generate_categories(config):
    ...
    max_cost = config.getfloat("maxCost")
    branch_cost = config.getfloat("branchCost")
    no_branch_cost = config.getfloat("noBranchCost")

    categories, costs = list(), list()
    pq = PriorityQueue()
    # put_index is for breaking ties in the priority queue
    put_index = 0
    start_t = CategoryTree(prim=0)
    start_cost = cat_cost(0, config) + no_branch_cost
    pq.put((start_cost, put_index, start_t))
    put_index += 1
    split_ta = CategoryTree(op="-a", res=start_t, arg=start_t)
    split_tb = CategoryTree(op="-b", res=start_t, arg=start_t)
    while len(categories) < num_cats: #and not pq.empty():
        cost, _, t = pq.get_nowait()
        if cost > max_cost:
            break
        ...
        for indices in powerset_minus_empty(tree_ixs):
            ...
            t_cost_incr = cost
            # cost change for replacing a single category k with a 
            # category k+1
            cost_change = cat_cost(new_prim, config) \
                - cat_cost(curr_max_prim, config)
            for ix in indices:
                t_copy_incr.replace_leaf(ix, new_prim_t)
                t_cost_incr += cost_change
            pq.put((t_cost_incr, put_index, t_copy_incr))
            put_index += 1
        ...
        for indices in powerset_minus_empty(zero_ixs):
            ...
            # (net) cost change for replacing a noBranch with a branch and
            # adding two noBranches each with category 0
            # TODO add in cost for -a and -b if nonzero
            cost_change = branch_cost + no_branch_cost \
                + 2*cat_cost(0, config)
            t_cost_split = cost
            for ix in indices:
                t_copy_split_a.replace_leaf(ix, split_ta)
                t_copy_split_b.replace_leaf(ix, split_tb)
                t_cost_split += cost_change
            pq.put((t_cost_split, put_index, t_copy_split_a))
            put_index += 1
            pq.put((t_cost_split, put_index, t_copy_split_b))
            put_index += 1
